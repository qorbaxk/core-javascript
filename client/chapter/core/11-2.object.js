...
text = "안녕 ㅋ"; // message와 text는 달라짐 참조복사는 원시데이터에는 영향 끼치지 않는다.

// conversationTool.name = "line"; // 둘다 line이 됨 같은 네임을 바라보게됨
...
// 3. 전개 연산자(...)를 사용한 복사
const spreadObject = { ...messenger }; //진짜 많이 쓴다
console.log("spread", spreadObject);

// 4. 객체를 복사해주는 유틸 함수
const utilObject = (object) => {
  let result = { ...object };
  return result;
};
console.log(utilObject(messenger));
