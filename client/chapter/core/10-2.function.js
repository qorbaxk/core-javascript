...
// 유명(이름을 가진) 함수 (표현)식
let namedFunctionExpression = function name() {};

// 콜백 함수 (표현)식
const callbackFunctionExpression = (url, resolve, reject) => {
  url && typeof url === "string" && url.match(/http.+www/)
    ? resolve(url)
    : reject();
};
...
const MASTER = (($) => {
  ...
  모듈로서의 활용
  정보은닉화 incapsulation: 외부의 접근을 차단
  일부 정보만 노출 */
