...
// e는 exponential의 약자 (지수)
// 앞에가 1이고 뒤에 0이 8개가 붙겠다 라는 뜻
riches = 1e8;
...
// 거듭제곱
let pow = Math.pow(2, 5); // 2**5
console.log("pow:", pow);

// 최소, 최대 값 사이 난수 반환 함수
let getRandomMinMax = (a, b) => {
  let maxValue, minValue;
  if (a >= b) {
    maxValue = Math.round(a);
    minValue = Math.round(b);
  } else {
    maxValue = Math.round(b);
    minValue = Math.round(a);
  }
  let result = Math.random() * (maxValue - minValue) + minValue;
  return result;
};
console.log("getRandomMinMax", getRandomMinMax(14, 10));
